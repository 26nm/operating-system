 ...
 * file storage, retrieval, and metadata organization.
 * 
 * This class handles:
 * - Initializing the file system by setting up the superblock, directory, and file table.
 * - Synchronizing file system data to disk to ensure persistence.
 * - Formatting the file system by resetting its structures and preparing it for use.
 * - Managing file operations, including opening, reading, writing, and closing files.
 * - Handling file metadata such as file sizes and seek pointers.
 * - Allocating and deallocating storage blocks for files, ensuring efficient space usage.
 * 
 * The file system interacts with a simulated disk, using inodes to track file data 
 * and employing direct and indirect block allocation methods to store file contents. 
 * Thread safety is ensured for key operations, and the system follows a structured 
 * approach to maintaining consistency between memory and disk.
 * 
 * Current Limitations:
 * - The system does not efficiently handle large file reads or writes due to 
 *   the constraints of its inode structure and block-based allocation system.
 * - Indirect block management is limited, making it difficult to handle files 
 *   that exceed the direct pointer storage capacity.
 * - Lack of caching optimizations may lead to slower performance when dealing 
 *   with multiple concurrent file operations.
 * - No support for dynamic resizing or extending file storage beyond the 
 *   predefined block allocation limits.
 * 
 * By: Nolan Dela Rosa
 * 
 * March 20, 2025
