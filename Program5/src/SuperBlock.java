 ...
 *
 * Overview:
 * - The superblock is a critical structure in the file system, located at Block #0, 
 *   storing essential metadata that describes the file system layout and status.
 * - It manages the total number of blocks on the disk, the blocks reserved for inode storage, 
 *   and the pointer to the free block list.
 * - The class provides methods to format the disk, synchronize the superblock, and efficiently 
 *   allocate and deallocate blocks from the free list to manage disk space.
 * 
 * Assumptions and Limitations:
 * - The superblock assumes a fixed block size and does not support dynamic resizing.
 * - The free block list is a simple linked list and may not be the most efficient method for 
 *   large file systems or systems requiring more advanced block allocation strategies.
 * - The class does not support recovery from disk corruption beyond basic validation.
 * 
 * Design:
 * - The superblock stores key attributes including the total block count, inode block count, 
 *   and free block list pointer, which are critical for file system management.
 * - It interacts with the file system by managing block allocation and deallocation 
 *   and ensuring file system consistency across system restarts.
 * - Methods for syncing and formatting the file system are designed to ensure that the 
 *   superblock's state is accurately reflected on disk, maintaining data integrity.
 * 
 * Functionality and Performance Considerations:
 * - The `sync()` method ensures that the superblock’s state is always written to disk, 
 *   which is crucial for consistency during system restarts.
 * - Block allocation and deallocation through `getFreeBlock` and `returnBlock` help 
 *   maintain efficient disk space usage by managing the free block list.
 * - The system relies on a simple free block list for space management, which could 
 *   lead to fragmentation over time, affecting performance in systems with a high 
 *   frequency of allocations and deallocations.
 * - File system formatting resets the superblock and inodes, effectively erasing the 
 *   existing file system structure, which may not be ideal for systems requiring data recovery.
 *
 * Key methods:
 ...
 *
 * The SuperBlock is a vital part of the file system, managing the integrity and 
 * free space of the file system, and ensuring the consistency of file system operations 
 * through careful management of block allocations.
 * 
 * By: Nolan Dela Rosa
 * 
 * March 20, 2025
...
public class SuperBlock {
     ...
     */
    public SuperBlock() {
       totalBlocks = Constants.TEST_DISK_SIZE;
    ...
    void format(int numInodeBlocks) {
        this.totalBlocks = Constants.TEST_DISK_SIZE;
